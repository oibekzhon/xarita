    ...
    .onSnapshot((snap) => {
      ...
      function rebuildFriendsList() {
        ...
        if (renderedCount === 0) {
          ...
          listEl.insertAdjacentHTML("beforeend", '<div class="friends-empty">Barcha do\'stlar hozir offline</div>');
        }
      }
...
document.getElementById("friendSearchBtn").addEventListener("click", async () => {
  ...
  document.getElementById("sendReqBtn").addEventListener("click", async () => {
    ...
    if (!q.empty) {
      result.textContent = "So'rov allaqachon yuborilgan.";
      return;
    }

    const fSnap = await db.collection("friends").doc(`${currentUser.uid}_${foundUid}`).get();
    if (fSnap.exists) {
      result.textContent = "Bu odam allaqachon do'stingiz!";
      return;
    }
